"""Print reconstructions from the Euclidean, vanilla-hyperbolic and A4-gc ImageNet RQ-VAE checkpoints.

Loads a fixed set of ImageNet val images, runs them through all three trained
RQVAE generators, and saves a comparison grid:
  row 0 = originals
  row 1 = Euclidean recon
  row 2 = vanilla hyperbolic recon
  row 3 = A4 (gradient-routed, gc) hyperbolic recon
"""
import os, sys, glob
import torch
...
CKPTS = [
    ...
    ("a4gc",      "/home/acolombo/VAEs/checkpoint/imagenet_rqvae_a4gc/latest.pth"),
]
OUT = "/home/acolombo/VAEs/figures/imagenet_recon_euc_hyp_a4.png"
N = 8
...
# Fixed, evenly-spaced val images for a reproducible panel (same picks as the
# original euc-vs-hyp figure).
...
# rows: [orig, euc, hyp, a4] each (N,3,256,256) in [-1,1]; lay out row-by-row.
grid = torch.cat(rows, dim=0)
grid = (grid + 1) / 2  # -> [0,1]
torchvision.utils.save_image(grid, OUT, nrow=N)
print("saved grid (rows: original / euclidean / hyperbolic / a4gc) ->", OUT, flush=True)
