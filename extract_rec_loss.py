...
for checkpoint in checkpoints:
    ...
    if train_matches and valid_matches:
        train_by_epoch = {int(e): float(l) for e, l in train_matches}
        valid_by_epoch = {int(e): float(l) for e, l in valid_matches}

        best_epoch_pattern = re.compile(r"<epoch:(\d+),[^\n]*?best_epoch:(\d+)>")
        best_epoch_matches = best_epoch_pattern.findall(content)
        best_epoch = int(best_epoch_matches[-1][1]) if best_epoch_matches else int(valid_matches[-1][0])

        best_rec_loss_train = train_by_epoch.get(best_epoch, float(train_matches[-1][1]))
        best_rec_loss_valid = valid_by_epoch.get(best_epoch, float(valid_matches[-1][1]))
        ...
        results[checkpoint] = {
            'train_rec_loss': best_rec_loss_train,
            'valid_rec_loss': best_rec_loss_valid,
...
if results:
    print("\n" + "="*70)
    print("TRAINING REC_LOSS RANKING (Best Model)")
    print("="*70)

    train_sorted = sorted(results.items(), key=lambda x: x[1]['train_rec_loss'])
    for rank, (checkpoint, data) in enumerate(train_sorted, 1):
        print(f"{rank:2d}. {checkpoint:40s} -> {data['train_rec_loss']:8.4f} (Best Epoch {data['best_epoch']})")

    print("\n" + "="*70)
    print("VALIDATION REC_LOSS RANKING (Best Model)")
    print("="*70)

    valid_sorted = sorted(results.items(), key=lambda x: x[1]['valid_rec_loss'])
    for rank, (checkpoint, data) in enumerate(valid_sorted, 1):
        print(f"{rank:2d}. {checkpoint:40s} -> {data['valid_rec_loss']:8.4f} (Best Epoch {data['best_epoch']})")
